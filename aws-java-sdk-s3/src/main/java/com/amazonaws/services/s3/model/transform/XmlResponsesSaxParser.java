...
import com.amazonaws.services.s3.model.metadata.CustomMetadataEntry;
import com.amazonaws.services.s3.model.metadata.CustomMetadataObject;
import com.amazonaws.services.s3.model.metadata.MetadataObject;
...
public class XmlResponsesSaxParser {
	...
	protected InputStream sanitizeXmlDocument(DefaultHandler handler, InputStream inputStream) throws IOException {
		...
		if (!sanitizeXmlDocument) {
			...
			try {
				...
				/*
				 * Read object listing XML document from input stream provided into a string
				 * buffer, so we can replace troublesome characters before sending the document
				 * to the XML parser.
				 */
				...
				/*
				 * Replace any carriage return (\r) characters with explicit XML character
				 * entities, to prevent the SAX parser from misinterpreting 0x0D characters as
				 * 0x0A and being unable to parse the XML.
				 */
	 ...
	 *
	 * See <a href=
	 * "https://www.owasp.org/index.php/XML_External_Entity_(XXE)_Prevention_Cheat_Sheet#XMLReader">OWASP
	 * XXE Cheat Sheet</a>
	 * 
	 * @param reader
	 *            the reader to disable the features on
	 ...
	 * @param s
	 *            The string to check.
	 * @return Null if the specified string was null, or empty, otherwise returns
	 ...
	 * NumberFormatException occurs while parsing the integer, an error is logged
	 * and -1 is returned.
	 *
	 * @param s
	 *            The string to parse and return as an integer.
	 *
	 * @return The integer value of the specified string, otherwise -1 if there were
	 *         any problems parsing the string as an integer.
	 */
	 ...
	 * NumberFormatException occurs while parsing the long, an error is logged and
	 * -1 is returned.
	 *
	 * @param s
	 ...
	 * @param inputStream
	 *
	 * @return true if the bucket's is configured as Requester Pays, false if it is
		...
		protected void doEndElement(String uri, String name, String qName) {
			if (atTopLevel()) {
				if (name.equals("ListBucketResult")) {
					/*
					 * S3 only includes the NextMarker XML element if the request specified a
				...
				if (name.equals("Name")) {
					...
					objectListing
							.setDelimiter(decodeIfSpecified(checkForEmptyString(getText()), shouldSDKDecodeResponse));
	...
	public static class ListObjectsV2Handler extends AbstractHandler {
		...
		protected void doEndElement(String uri, String name, String qName) {
			if (atTopLevel()) {
				if (name.equals("ListBucketResult")) {
					/*
					 * S3 only includes the NextContinuationToken XML element if the request
					 * specified a delimiter, but for consistency we'd like to always give easy
					 * access to the next token if we're returning a list of results that's
					 * truncated.
					 */
					if (result.isTruncated() && result.getNextContinuationToken() == null) {
						...
						if (!result.getObjectSummaries().isEmpty()) {
							nextContinuationToken = result.getObjectSummaries()
									.get(result.getObjectSummaries().size() - 1).getKey();
	...
	/**
	 * Handler for ListAllMyBuckets response XML documents. The document is parsed
	 * into {@link Bucket}s available via the {@link #getBuckets()} method.
	 */
	...
	/**
	 * Handler for AccessControlList response XML documents. The document is parsed
	 * into an {@link AccessControlList} object available via the
			...
			if (in("AccessControlPolicy")) {
				...
				if (name.equals("Grantee")) {
					...
					if ("AmazonCustomerByEmail".equals(type)) {
						...
						/*
						 * Nothing to do for GroupGrantees here since we can't construct an empty enum
						 * value early.
						 */
					}
				}
		...
		protected void doEndElement(String uri, String name, String qName) {
			if (in("AccessControlPolicy", "Owner")) {
				...
				if (name.equals("ID")) {
					...
					/*
					 * Only GroupGrantees contain an URI element in them, and we can't construct
					 * currentGrantee during startElement for a GroupGrantee since it's an enum.
					 */
	...
	/**
	 * Handler for LoggingStatus response XML documents for a bucket. The document
	 * is parsed into an {@link BucketLoggingConfiguration} object available via the
	...
	/**
	 * Handler for parsing RequestPaymentConfiguration XML response associated with
	 * an Amazon S3 bucket. The XML response is parsed into a
		...
		protected void doEndElement(String uri, String name, String qName) {
			...
			if (in("ListVersionsResult")) {
				if (name.equals("Name")) {
					...
					versionListing
							.setPrefix(decodeIfSpecified(checkForEmptyString(getText()), shouldSDKDecodeResponse));
				} else if (name.equals("KeyMarker")) {
					versionListing
							.setKeyMarker(decodeIfSpecified(checkForEmptyString(getText()), shouldSDKDecodeResponse));
	 ...
	 * Uuag1LuByRx9e6j5Onimru9pO4ZVKnJ2Qz7/C1NPcfTWAtRPfTaOFg== x-amz-request-id:
	 * 656c76696e6727732072657175657374 Date: Tue, 16 Feb 2010 20:34:56 GMT
	 * Content-Length: 1330 Connection: keep-alive Server: AmazonS3
	 *
	 ...
	 * <StorageClass>STANDARD</StorageClass> <Initiated>Tue, 26 Jan 2010 19:42:19
	 * GMT</Initiated> </Upload> <Upload> <Key>my-movie.m2ts</Key>
	 * <UploadId>VXBsb2FkIElEIGZvciBlbHZpbmcncyBteS1tb3ZpZS5tMnRzIHVwbG9hZA</
	 * UploadId> <Owner> <ID>b1d16700c70b0b05597d7acd6a3f92be</ID>
	 * <DisplayName>delving</DisplayName> </Owner>
	 * <StorageClass>STANDARD</StorageClass> <Initiated>Tue, 16 Feb 2010 20:34:56
	 * GMT</Initiated> </Upload> <Upload> <Key>my-movie.m2ts</Key>
	 * <UploadId>YW55IGlkZWEgd2h5IGVsdmluZydzIHVwbG9hZCBmYWlsZWQ</UploadId> <Owner>
	 * <ID>b1d16700c70b0b05597d7acd6a3f92be</ID> <DisplayName>delving</DisplayName>
	 * </Owner> <StorageClass>STANDARD</StorageClass> <Initiated>Wed, 27 Jan 2010
	 * 03:02:01 GMT</Initiated> </Upload> <CommonPrefixes> <Prefix>photos/</Prefix>
	 * </CommonPrefixes> <CommonPrefixes> <Prefix>videos/</Prefix> </CommonPrefixes>
	 ...
	 * <ID>x1x16700c70b0b05597d7ecd6a3f92be</ID> <DisplayName>username</DisplayName>
	 * </Initiator> <StorageClass>STANDARD</StorageClass>
	 ...
	 * <ETag>"7778aef83f66abc1fa1e8477f296d394"</ETag> <Size>10485760</Size> </Part>
	 * <Part> <PartNumber>3</PartNumber> <LastModified>Wed, 27 Jan 2010 03:02:02
	 * GMT</LastModified> <ETag>"aaaa18db4cc2f85cedef654fccc4a4x8"</ETag>
	 * <Size>10485760</Size> </Part> </ListPartsResult>
	 */
	...
	/**
	 * Handler for parsing the get replication configuration response from Amazon
	 * S3. Sample HTTP response is given below.
	 *
	 * <pre>
	 ...
	 *
	 * <Tagging> <TagSet> <Tag> <Key>Foo</Key> <Value>1</Value> </Tag> <Tag>
	 * <Key>Bar</Key> <Value>2</Value> </Tag> <Tag> <Key>Baz</Key> <Value>3</Value>
	 * </Tag> </TagSet> </Tagging>
	 */
	 ...
	 * HTTP/1.1 200 OK x-amz-id-2:
	 * Uuag1LuByRx9e6j5Onimru9pO4ZVKnJ2Qz7/C1NPcfTWAtRPfTaOFg== x-amz-request-id:
	 * 656c76696e6727732072657175657374 Date: Tue, 20 Sep 2012 20:34:56 GMT
	 * Content-Type: application/xml Transfer-Encoding: chunked Connection:
	 * keep-alive Server: AmazonS3
	 * 
	 * <?xml version="1.0" encoding="UTF-8"?> <DeleteResult> <Deleted>
	 * <Key>Key</Key> <VersionId>Version</VersionId> </Deleted> <Error>
	 * <Key>Key</Key> <VersionId>Version</VersionId> <Code>Code</Code>
	 ...
	 * <Status>Enabled</Status> <Filter> <Prefix>logs/</Prefix> <Tag>
	 * <Key>key1</Key> <Value>value1</Value> </Tag> <And> <Prefix>logs/</Prefix>
	 * <Tag> <Key>key1</Key> <Value>value1</Value> </Tag> <Tag> <Key>key1</Key>
	 * <Value>value1</Value> </Tag> </And> </Filter> <Transition> <Days>30</Days>
	 * <StorageClass>STANDARD_IA</StorageClass> </Transition> <Transition>
	 * <Days>90</Days> <StorageClass>GLACIER</StorageClass> </Transition>
	 * <Expiration> <Days>365</Days> </Expiration> <NoncurrentVersionTransition>
	 ...
	 * <Rule> <ID>image-rule</ID> <Prefix>image/</Prefix> <Status>Enabled</Status>
	 * <Transition> <Date>2012-12-31T00:00:00.000Z</Date>
	...
	public static class BucketLifecycleConfigurationHandler extends AbstractHandler {
		...
		private final BucketLifecycleConfiguration configuration = new BucketLifecycleConfiguration(
				new ArrayList<Rule>());
	 ...
	 * HTTP/1.1 200 OK x-amz-id-2:
	 * Uuag1LuByRx9e6j5Onimru9pO4ZVKnJ2Qz7/C1NPcfTWAtRPfTaOFg== x-amz-request-id:
	 * 656c76696e6727732072657175657374 Date: Tue, 20 Sep 2011 20:34:56 GMT
	 * Content-Length: Some Length Connection: keep-alive Server: AmazonS3
	...
	public static class BucketCrossOriginConfigurationHandler extends AbstractHandler {
		...
		private final BucketCrossOriginConfiguration configuration = new BucketCrossOriginConfiguration(
				new ArrayList<CORSRule>());
	 ...
	 * x-amz-request-id: 51991C342C575321 Date: Wed, 14 May 2014 02:11:22 GMT
	 * Server: AmazonS3 Content-Length: ...
	 * 
	 * <?xml version="1.0" encoding="UTF-8"?> <MetricsConfiguration
	 * xmlns="http://s3.amazonaws.com/doc/2006-03-01/"> <Id>metrics-id</Id> <Filter>
	 * <!-- A filter should have only one of Prefix, Tag or And predicate. ->
	 * <Prefix>prefix</Prefix> <Tag> <Key>Project</Key> <Value>Foo</Value> </Tag>
	 * <And> <Prefix>documents/</Prefix> <Tag> <Key>foo</Key> <Value>bar</Value>
	 ...
	 * xmlns="http://s3.amazonaws.com/doc/2006-03-01/"> <MetricsConfiguration> ...
	 * </MetricsConfiguration> <IsTruncated>true</IsTruncated>
	 ...
	 * Server: AmazonS3 Content-Length: ...
	 * 
	 * <?xml version="1.0" encoding="UTF-8"?> <AnalyticsConfiguration
	 * xmlns="http://s3.amazonaws.com/doc/2006-03-01/"> <Id>XXX</Id> <Filter> <And>
	 * <Prefix>documents/</Prefix> <Tag> <Key>foo</Key> <Value>bar</Value> </Tag>
	 * </And> </Filter> <StorageClassAnalysis> <DataExport>
	 ...
	 * </S3BucketDestination> </Destination> </DataExport> </StorageClassAnalysis>
	 ...
	 * xmlns="http://s3.amazonaws.com/doc/2006-03-01/"> <AnalyticsConfiguration> ...
	 * </AnalyticsConfiguration> <IsTruncated>true</IsTruncated>
	 * <ContinuationToken>token1</ContinuationToken>
	 * <NextContinuationToken>token2</NextContinuationToken>
	 * </ListBucketAnalyticsConfigurationsResult>
	 */
	 ...
	 * <Destination> <S3BucketDestination> <AccountId>A2OCNCIEQW9MSG</AccountId>
	 * <Bucket>s3-object-inventory-list-gamma-us-east-1</Bucket>
	 * <Format>CSV</Format> <Prefix>string</Prefix> </S3BucketDestination>
	 * </Destination> <IsEnabled>true</IsEnabled> <Filter> <Prefix>string</Prefix>
	 * </Filter> <Id>configId</Id>
	 ...
	 * </InventoryConfiguration> <IsTruncated>true</IsTruncated>
	 * <ContinuationToken>token1</ContinuationToken>
	 * <NextContinuationToken>token2</NextContinuationToken>
	 * </ListInventoryConfigurationsResult>
	 */
	...
	public SearchMetadataHandler parseSearchMetadataResponse(InputStream inputStream,
			final boolean shouldSDKDecodeResponse) throws IOException {
		SearchMetadataHandler handler = new SearchMetadataHandler(shouldSDKDecodeResponse);
		parseXmlInputStream(handler, sanitizeXmlDocument(handler, inputStream));

		return handler;
	}

	public static class SearchMetadataHandler extends AbstractHandler {
		private final SearchMetadataResponse searchMetadataResponse = new SearchMetadataResponse();
		private final boolean shouldSDKDecodeResponse;

		private MetadataObject currentObject = null;
		private Owner currentOwner = null;
		private String lastKey = null;
		private CustomMetadataObject customMetadataObject = null;
		private CustomMetadataEntry customMetadataEntry = null;
		
		public SearchMetadataHandler(final boolean shouldSDKDecodeResponse) {
			this.shouldSDKDecodeResponse = shouldSDKDecodeResponse;
		}

		public SearchMetadataResponse getSearchMetadataResponse() {
			return searchMetadataResponse;
		}

		@Override
		protected void doStartElement(String uri, String name, String qName, Attributes attrs) {
			if (in("SearchMetadataResponse")) {
				if (name.equals("Contents")) {
					currentObject = new MetadataObject();
				}
			} else if (in("SearchMetadataResponse", "Contents")) {
				if (name.equals("Owner")) {
					currentOwner = new Owner();
				} else if (name.equals("CustomMetadata")) {
					customMetadataObject = new CustomMetadataObject();
				}
			}  else if (in("SearchMetadataResponse", "Contents", "CustomMetadata")) {
				if (name.equals("Entry")) {
					customMetadataEntry = new CustomMetadataEntry();
				}
			}
		}
		...
		protected void doEndElement(String uri, String name, String qName) {
			if (atTopLevel()) {
				if (name.equals("Contents")) {
					 ...
					 * next marker if we're returning a list of results that's truncated.
					 */
					if (searchMetadataResponse.isTruncated() && searchMetadataResponse.getNextMarker() == null) {

						String nextMarker = null;
						if (!searchMetadataResponse.getObjects().isEmpty()) {
							nextMarker = searchMetadataResponse.getObjects()
									.get(searchMetadataResponse.getObjects().size() - 1).getKey();
						...
						searchMetadataResponse.setNextMarker(nextMarker);
					}
				}
			}

			else if (in("SearchMetadataResponse")) {
				if (name.equals("Marker")) {
					searchMetadataResponse
							.setMarker(decodeIfSpecified(checkForEmptyString(getText()), shouldSDKDecodeResponse));

				} else if (name.equals("IsTruncated")) {
					String isTruncatedStr = StringUtils.lowerCase(getText());

					if (isTruncatedStr.startsWith("false")) {
						searchMetadataResponse.setTruncated(false);
					} else if (isTruncatedStr.startsWith("true")) {
						searchMetadataResponse.setTruncated(true);
					} else {
						throw new IllegalStateException("Invalid value for IsTruncated field: " + isTruncatedStr);
					}

				} else if (name.equals("Contents")) {
					searchMetadataResponse.getObjects().add(currentObject);
					currentObject = null;
				}
			}
				...
				if (name.equals("Bucket")) {
					currentObject.setBucketName(decodeIfSpecified(getText(), shouldSDKDecodeResponse));
					...
					currentObject.setInstance(decodeIfSpecified(getText(), shouldSDKDecodeResponse));

				} else if (name.equals("VersionedEpoch")) {
					currentObject
							.setVersionedEpoch(Integer.parseInt(decodeIfSpecified(getText(), shouldSDKDecodeResponse)));
					...
					currentObject.setContentType(decodeIfSpecified(lastKey, shouldSDKDecodeResponse));
					...
					currentObject.setCustomMetadata(customMetadataObject);
					customMetadataObject = null;
				}
			}

			else if (in("SearchMetadataResponse", "Contents", "Owner")) {
				if (name.equals("ID")) {
					currentOwner.setId(getText());
				...
				if (name.equals("Entry")) {
					customMetadataObject.getMetadataEntries().add(customMetadataEntry);
					customMetadataEntry = null;

				}
			}
			
			else if (in("SearchMetadataResponse", "Contents", "CustomMetadata", "Entry")) {
				if (name.equals("Name")) {
					customMetadataEntry.setName(getText());
				} else if (name.equals("Value")) {
					customMetadataEntry.setValue(getText());
				}
			}
